...
internal func bootstring_encode(_ symbol: String) -> String? {
  var encoded = ""
  let success = LDHBootstring.encode(symbol.unicodeScalars) { ascii in
    encoded.unicodeScalars.append(Unicode.Scalar(ascii))
  }
  guard success else {
...
// --------------------------------------------
// FIXME: Comments and documentation below all refer to 'Punycode'
// --------------------------------------------


private enum LDHBootstring {}

extension LDHBootstring {
      ...
      for scalar in source {
        if scalar.value < codePoint || isBasic(scalar) {
          ...
          while true {
            ...
            if q < t {
              writeASCII(LDHBootstring.encode_digit(q))
              break
            }
            let digit: UInt32
            (q, digit) = (q &- t).quotientAndRemainder(dividingBy: Base &- t)
            writeASCII(LDHBootstring.encode_digit(digit &+ t))
            k &+= LDHBootstring.Base
          }
